...
FROM fqz_order_related_graph_bu a     --order订单表现
...
union all 
-- 一度_ID数量    
SELECT a.order_src,'id_cnt' title,count(distinct a.cert_no1) cnt FROM fqz_order_related_graph_bu a     --order订单表现
...
union all
-- 一度关联自身_黑合同数量
SELECT a.order_src,'black_cnt' title,count(distinct a.order1) cnt FROM fqz_order_related_graph_bu a     --order订单表现
where a.degree_type='1' and a.apply_time_src>a.apply_time1
and a.cert_no_src <> a.cert_no1 and a.label1 = 1
group by a.order_src
-- 一度_Q标拒绝数量  1  
union all 
SELECT a.order_src,'q_refuse_cnt' title,count(distinct a.order1) cnt FROM fqz_order_related_graph_bu a     --order订单表现  
...
union all
-- 一度_通过合同数量  2  
SELECT a.order_src,'pass_cnt' title,count(distinct a.order1) cnt FROM fqz_order_related_graph_bu a     --order订单表现
