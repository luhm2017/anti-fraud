...
import com.lakala.finance.anti_fraud.common.EnvUtil
...
object AntiFraudScoreBYSparkSQL /*extends Logging*/{
  ...
  def main(args: Array[String]): Unit = {
    ...
    //processVariableByTempTable(year,month,day)
  ...
  //fqz score
  def predictScore(database:String,table:String,data:String,cnt:String,path:String): Unit ={
    //随机取label=0数据
  ...
  def predictScore(database:String,table:String,path:String,host:String,user:String,
    ...
    //加载模型，目前只考虑gbdt
    val model = GradientBoostedTreesModel.load(sc,s"$path/model/gbdt")
    //打分数据
  ...
  //根据modelType评分
  def predictScore(modelType:String,database:String,table:String,path:String): Unit ={
    //实时数据
    val dataInstance = hc.sql(s"select * from $database.$table").map {
      row =>
        val arr = new ArrayBuffer[Double]()
    ...
    if(modelType.equals("RF")){
      val model = RandomForestModel.load(sc,s"$path/model/rf")
      //打分
      ...
      //preditData保存
      preditDataRF.saveAsTextFile(s"$path/predictionAndLabels/rf")
    }
  ...
  def FS2JDBC(path:String): Unit ={
    ...
    //preditDataGBDT
    val input = sqlContext.read.load(s"$path/fqz0720/predictionAndLabels/gbdt")
